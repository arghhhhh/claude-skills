...
- **Port 9876 — keep Houdini-MCP off it.** Houdini-MCP also defaults to `localhost:9876`; if both bind it, `houdini.*`/`blender.*` calls cross-answer (a `houdini.*` call may return this Blender scene). Resolved in this setup by pinning Houdini to **9877** (`HOUDINIMCP_PORT`); Blender keeps 9876. If you spin up another 9876 MCP, expect the same collision.
