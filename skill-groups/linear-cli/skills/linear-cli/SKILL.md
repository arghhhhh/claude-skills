...
Drive [Linear.app](https://linear.app) from the terminal with `linear` ([schpet/linear-cli](https://github.com/schpet/linear-cli)). Git/jj-aware and agent-friendly (structured JSON on many read commands). Documented against **CLI v2.3.0** — verify with `linear --version` if flags mismatch.
...
- **Configure project defaults**: `linear config` (interactive; alias `linear configure`) writes `.linear.toml`. Valid keys: `team_id`, `workspace`, `api_key`, `issue_sort` (`manual`|`priority`), `vcs` (`git`|`jj`), `download_images`, `hyperlink_format`, `attachment_dir`, `auto_download_attachments`, `issue_create_ask_project` (bool — prompt for a project during interactive `issue create`), `issue_create_assign_self` (`always`|`auto`|`never`, default `auto` — self-assign behavior on create). Lookup order: `./linear.toml` / `./.linear.toml` (cwd), git root (also `<gitroot>/.config/linear.toml`), then global `~/.config/linear/linear.toml` (Windows: `%APPDATA%\linear\linear.toml`).
...
linear issue create [-t "title"] [-d "desc" | --description-file spec.md] \
                    [--team ENG] [--project "Name"] [--milestone "Phase 1"] [--cycle active|next|+1] \
                    [-p 1-4] [-a self] [-l bug] [-s "In Progress"] [--parent ENG-100] \
                    [--estimate N] [--due-date <date>] [--start] \
                    [--no-use-default-template] [--no-interactive]
                    # --cycle accepts relative refs (active/now, next, previous, +1, --cycle=-1).
                    # no --content flag here (that's project create); use -d/--description-file.
linear issue update [ISSUE] [-t "..."] [-d "..." | --description-file f.md] [--team ENG] \
                    [--project "Name"] [--milestone "Phase 2"] [--cycle active|next|+1] [--clear-cycle] \
                    [-p 1-4] [-a <user> | --unassign] [-l bug] [-s <state>] [--parent <id>] \
                    [--estimate N] [--due-date <date>]
                    # --unassign clears assignee (not with -a). --clear-cycle removes from cycle.
                    # --cycle accepts relative refs: active/now, next, previous, +1, --cycle=-1
...
linear issue comment add [ISSUE] [-b "text" | --body-file f.md] [-p <parentCommentId>] [-a <filepath>] [--public]
                  # body flag is -b/--body, NOT -d. -p to reply; -a attaches files (repeatable, images render inline).
                  # Uploads are private (workspace-only) by default; --public gives an unauthenticated image URL.
linear issue comment update|delete <commentId>
linear issue attach <ISSUE> <filepath> [-t "title"] [-c "comment body"] [--public]
                  # sidebar attachment (not inline). --public = unauthenticated image URL (default private).
                  # for an INLINE image, prefer `issue comment add --attach` instead.
...
linear project create -n "Name" -t ENG [-d "desc" | -f desc.md] [-l @me] \
                      [-s planned|started|paused|completed|canceled|backlog] \
                      [--content "# Overview" | --content-file overview.md] [--label <lbl>] \
                      ...
                      # -d/-f = short description (255-char API cap); --content/--content-file = long overview.
                      # --label repeatable. --project/--milestone/--initiative accept UUID, slug ID, or name.
linear project update <projectId> [-n ...] [-d ... | -f desc.md] [-s <status>] [-l <lead>] \
                      [--label <lbl>] [--start-date ...] [--target-date ...] [-t ENG]
                      # --label REPLACES labels (repeatable to set several).
...
linear milestone view <milestoneId> [--all]   # default lists first 10 issues; --all paginates the full set
...
linear document update <id> [-t "..."] [-c "..." | -f f.md] [--project <slugOrId>] [--icon 📄] [-e] [--force]
                       # -e opens $EDITOR; --project re-attaches to a project.
                       # content edits are BLOCKED when the doc has inline comments (they'd lose anchors) — --force overrides.
...
- **Post an inline screenshot on an issue**: `linear issue comment add ENG-123 -b "repro:" --attach shot.png` (renders inline; add `--public` only if the image must be viewable without workspace auth). `issue attach` puts it in the sidebar instead, not inline.
